...
const About = () => {
    const [ref, inView] = useInView({
        triggerOnce: true,
        threshold: 0.2, // Définir le seuil d'intersection ici selon vos besoins
    });

    const props = useSpring({
        opacity: inView ? 1 : 0,
        transform: inView ? 'translateY(0px)' : 'translateY(100px)',
        config: { duration: 800 }, // Augmentez la durée de transition en millisecondes
    });
    useEffect(() => {
    ...
    return (
        <div className='aboutDescription' id='About' ref={ref}>
